...
- (void)viewDidLoad
{
    ...
    self.searchBar.tintColor = [UIColor colorWithRed:0.7 green:0.7 blue:0.7 alpha:1];
...
- (void)refreshScopeTitles {
    ...
    NSArray *listItems = [desc componentsSeparatedByString:@": "];
    NSString *className = listItems[0];
   
    NSArray *structs = [[NSArray alloc] initWithObjects:@"NSRect", nil]; 
    BOOL isStruct = [structs containsObject:className];

    NSMutableArray *scopes = [NSMutableArray arrayWithObject:@"Base"];
    if (parent && !isStruct) {
        [scopes addObject:@"+ Parent"];
    }
    if (parentSuper && parentSuper != [NSObject class] && !isStruct) {
        [scopes addObject:@"+ Inherited"];
    }
    if ([self.object isKindOfClass:[NSObject class]]  && !isStruct) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell) {
        if (useDescriptionCell) {
            ...
            cell.detailTextLabel.textColor = [UIColor colorWithRed:0.7 green:0.7 blue:0.7 alpha:1];
